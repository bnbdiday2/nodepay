    ...
    with ThreadPoolExecutor(max_workers=30) as executor:
        try:
            response = await loop.run_in_executor(
                executor,
                lambda: scraper.post(url, json=data, headers=headers, timeout=30)
            )
    ...
    try:
        ...
        if response["code"] == 0:
            ...
            if '@' in proxy:
                ...
                proxy_ip = proxy.split('://')[-1].split(':')[0]
            logger.info(f"{Fore.GREEN}Ping successful for token {account_info.token}{Style.RESET_ALL}")
            # logger.info(f"{Fore.GREEN}Ping successful for token {account_info.token} using proxy IP {proxy_ip}{Style.RESET_ALL}")
